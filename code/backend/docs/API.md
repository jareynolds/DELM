...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `symbol_type` | string | Yes | - | See available types below |
...
- **Shapes**: `circle`, `square`, `triangle`, `diamond`, `hexagon`, `pentagon`, `octagon`
- **Decorative**: `star`, `heart`, `shield`, `badge`, `burst`, `ribbon`
- **Directional**: `arrow`, `chevron`
- **UI Elements**: `ring`, `divider`, `dots`, `wave`, `lightning`, `cross`

All symbols use gradient fills with primary and secondary colors for visual appeal.

**Example**
```bash
# Generate a star symbol
curl -X POST http://127.0.0.1:3005/generate/symbol \
  -H "Content-Type: application/json" \
  -d '{"symbol_type": "star", "size": 128, "primary_color": "#f59e0b"}'

# Generate a shield symbol
curl -X POST http://127.0.0.1:3005/generate/symbol \
  -H "Content-Type: application/json" \
  -d '{"symbol_type": "shield", "primary_color": "#10b981", "secondary_color": "#6ee7b7"}'

# Generate a lightning bolt
curl -X POST http://127.0.0.1:3005/generate/symbol \
  -H "Content-Type: application/json" \
  -d '{"symbol_type": "lightning", "primary_color": "#f59e0b"}'
```
