...
export const Badge: React.FC<BadgeProps> = ({
  ...
  return (
    ...
    # ========================================
    # Test 10: Generate Component Mockup (no LLM)
    # ========================================
    print_header("10. Generate Component Mockup")
    ...
    success, data = test_endpoint(
        ...
        "/generate/mockup",
        {
            ...
            "props": {"text": "Click Me", "color": "#3b82f6"},
            "width": 400,
            "height": 200
        }
    )
    results.append(("Generate mockup", success))
    if success:
        print(f"  Image size: {data.get('width')}x{data.get('height')}")
        print(f"  Base64 length: {len(data.get('image', ''))} chars")
        print(f"  Has data URL: {'Yes' if data.get('data_url') else 'No'}")

    # Test card mockup
    print()
    success, _ = test_endpoint(
        "Generate card mockup",
        "POST",
        "/generate/mockup",
        {
            "component_type": "card",
            "props": {"title": "Test Card", "content": "This is test content"}
        }
    )
    ...
    # ========================================
    # Test 11: Code to Image
    # ========================================
    print_header("11. Code to Image")
    ...
    success, data = test_endpoint(
        ...
        "/generate/code-to-image",
        {
            "code": """<div class="p-8 bg-gray-100">
                ...
                <p class="mt-2 text-gray-600">This is a test render</p>
                <button class="mt-4 px-4 py-2 bg-blue-500 text-white rounded">Click</button>
            </div>""",
            "width": 600,
            "height": 300
        }
    )
    results.append(("Code to image", success))
    if success:
        print(f"  Image size: {data.get('width')}x{data.get('height')}")
        print(f"  Base64 length: {len(data.get('image', ''))} chars")
